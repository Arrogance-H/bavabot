...
### 与M用户同步功能的交互 / Interaction with M User Sync

当使用M用户管理面板的 `sync` 命令从数据库同步M尊享用户时，系统会自动过滤掉排除列表中的用户：

**行为 / Behavior:**
- 从数据库获取所有 `lv='m'` 的用户
- **自动过滤掉在 `m_welcome_exclude` 列表中的用户** 🚫
- 只将不在排除列表中的用户添加到 `m_users`
- 显示同步的用户数和跳过的用户数

**示例 / Example:**
```
数据库中有5个M尊享用户：[111, 222, 333, 444, 555]
排除列表中有2个用户：[222, 444]
