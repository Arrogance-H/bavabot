...
async def format_demand_records(current_page=1, current_filter="all"):
    ...
    try:
        ...
        for idx, record in enumerate(page_records):
            ...
            # 从批量获取的结果中获取TG昵称
            user_name = tg_users_map.get(record.tg, f"用户{record.tg}")
...
@bot.on_message(filters.command('mydemand', prefixes))
async def mydemand_command(_, msg):
    """
    用户查看自己的ME点播记录命令
...
async def format_my_demands(user_records, current_page=1):
    """格式化用户自己的点播记录显示"""
    try:
        total_records = len(user_records)
        total_pages = max(1, math.ceil(total_records / RECORDS_PER_PAGE))
        ...
        page_row = []
        if current_page > 1:
            page_row.append(InlineKeyboardButton("⬅️ 上页", callback_data=f"mydemand_page_{current_page-1}"))
        if current_page < total_pages:
            page_row.append(InlineKeyboardButton("下页 ➡️", callback_data=f"mydemand_page_{current_page+1}"))
        ...
        # 刷新按钮
        keyboard_buttons.append([
            InlineKeyboardButton("🔄 刷新", callback_data="mydemand_refresh")
        ])
...
@bot.on_callback_query(filters.regex(r'^mydemand_page_(\d+)$'))
async def handle_mydemand_page(_, call):
    """处理我的点播记录分页请求"""
    try:
        page = int(call.matches[0].group(1))
        ...
        if not user_records:
            await callAnswer(call, "您还没有点播记录", True)
            return
        
        # 按时间排序，最新的在前
        user_records.sort(key=lambda x: x.create_at, reverse=True)
        
        text, keyboard = await format_my_demands(user_records, page)
        await editMessage(call, text, buttons=keyboard)
        await callAnswer(call, f"已切换到第{page}页")
...
@bot.on_callback_query(filters.regex(r'^mydemand_refresh$'))
async def handle_mydemand_refresh(_, call):
    """处理我的点播记录刷新请求"""
    try:
        user_id = call.from_user.id
