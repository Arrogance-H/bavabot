...
from bot import bot, moviepilot, tmdb, bot_photo, LOGGER, sakura_b
from bot.func_helper.msg_utils import callAnswer, editMessage, sendMessage, sendPhoto, callListen
from bot.func_helper.filters import user_in_group_on_filter
from bot.func_helper.fix_bottons import re_download_center_ikb, back_members_ikb, continue_search_ikb, request_record_page_ikb, mp_search_page_ikb, tmdb_search_page_ikb, tmdb_search_result_ikb
...
async def tmdb_search_handler(_, call):
    ...
    await callAnswer(call, '🎬 TMDB搜索')
    await editMessage(call, '🎬 **TMDB影视搜索**\n\n请在120s内发送你想搜索的电影或电视剧名称\n输入 /cancel 取消操作')

    txt = await callListen(call, 120, buttons=re_download_center_ikb)
    if txt is False:
        return
    if txt.text == '/cancel':
        await asyncio.gather(txt.delete(), editMessage(call, '🔍 已取消操作', buttons=re_download_center_ikb))
        return
...
async def tmdb_search_results(call, query: str, page: int = 1):
    """显示TMDB搜索结果"""
    try:
        await editMessage(call, '🔍 正在TMDB搜索中，请稍后...')
        
        success, results = await tmdb_service.search_multi(query, page)
        if not success or not results:
            await editMessage(call, '🤷‍♂️ 未找到相关影视作品，请尝试其他关键词', buttons=re_download_center_ikb)
            return

        # 计算分页信息
        total_pages = 10  # TMDB API通常支持多页结果
        has_prev = page > 1
        ...
        await editMessage(call, '❌ 搜索过程中出错，请稍后再试', buttons=re_download_center_ikb)
...
async def tmdb_select_item(_, call):
    ...
    if not results:
        await editMessage(call, '❌ 没有可选择的影片', buttons=re_download_center_ikb)
        return

    await editMessage(call, '请在120s内发送你要选择的影片编号（1-10）\n输入 /cancel 取消操作')
    ...
    try:
        ...
        if index < 1 or index > min(10, len(results)):
            await editMessage(call, '❌ 编号无效，请输入1-10之间的数字', buttons=re_download_center_ikb)
            return
        ...
        await editMessage(call, '❌ 请输入有效的数字编号', buttons=re_download_center_ikb)
    except Exception as e:
        LOGGER.error(f"选择TMDB影片出错: {str(e)}")
        await editMessage(call, '❌ 选择过程中出错', buttons=re_download_center_ikb)
...
async def show_tmdb_item_details(call, item: dict):
    ...
    if vote_average > 0:
        ...
        detail_text += f"⭐ **评分**: {vote_average:.1f}/10 {stars} ({vote_count}票)\n"
        
    detail_text += f"\n📝 **简介**: {overview}\n\n"
    detail_text += "接下来将使用此影片名称搜索资源站点"
    
    # 使用影片标题进行后续的资源搜索
    search_title = f"{title} {year}" if year else title
    
    # 保存选中的影片信息到用户搜索数据中
    user_search_data[call.from_user.id] = search_title
...
@bot.on_callback_query(filters.regex('^tmdb_search_resources$') & user_in_group_on_filter)
async def tmdb_search_resources(_, call):
    """使用TMDB选中的影片搜索资源"""
    search_text = user_search_data.get(call.from_user.id)
    if not search_text:
        await editMessage(call, '❌ 未找到搜索记录，请重新搜索', buttons=re_download_center_ikb)
        return
    
    await callAnswer(call, '🔍 开始搜索资源')
    
    # 先查询emby库中是否存在
    await editMessage(call, '🔍 正在查询Emby库，请稍后...')
    emby_results = await emby.get_movies(title=search_text)
    if emby_results:
        text = "🎯 Emby库中已存在以下相关资源:\n\n"
        for item in emby_results:
            text += f"• {item['title']} ({item['year']})\n"
...
async def cancel_tmdb_search(_, call):
    ...
    user_search_data.pop(call.from_user.id, None)
    await editMessage(call, '🔍 已取消TMDB搜索', buttons=re_download_center_ikb)