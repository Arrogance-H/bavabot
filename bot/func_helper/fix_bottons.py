...
def config_preparation() -> InlineKeyboardMarkup:
    ...
    keyboard = ikb(
        [[('📄 导出日志', 'log_out'), ('📌 设置探针', 'set_tz')],
         ...
         [(f'{lottery_status} 抽奖功能', 'set_lottery_status'), ('🎊 抽奖管理', 'lottery_manage')],
...
def lottery_manage_ikb():
    """抽奖管理面板按钮"""
    keyboard = ikb([
        [('🎊 创建抽奖', 'lottery_create_panel'), ('🎁 添加奖品', 'lottery_add_prize_panel')],
        [('📋 查看抽奖', 'lottery_list'), ('🎲 管理抽奖', 'lottery_admin')],
        [('🔙 返回', 'back_config')]
    ])
    return keyboard